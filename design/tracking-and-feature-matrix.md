...
| FR-001.3 | Progress + resume + deletion handling | Yes | Partial | Done: progress, skip unchanged scan work, deletion cleanup, and restart-safe compressed-render reruns that reuse finished cache files while retrying incomplete outputs. Missing: resumable in-flight job state across app restarts. |
...
| FR-050 | Playback state machine | Yes | Partial | Done: sequential renderer playback with resume state, start/end trims applied, playlist-click start from the selected playlist track in Preparation mode even during active main playback, playlist-click start in Live mode when idle, natural continuation through the remaining sequence, and scheduled cortina/tanda gap handling. Missing: deterministic engine abstraction, multi-client sync, richer scheduling visibility. |
...
| FR-094 | Alias metadata | Yes | Partial | Done: seeded alias dataset and registry editing UI; alias/canonical matching used in search ranking and orchestra grouping. Missing: backend persistence/sync of user-edited alias registry beyond local UI storage. |
...
| UI-012 | Playlist timeline | Yes | Partial | Done: active tanda expansion, active track highlight, estimated start times, idle auto-centering of the current tanda during playback, fallback to tanda stored duration when per-track analysis duration is missing, auto-fill placeholder tandas (required size/style) with mismatch warnings using 9-minute assumed duration for end-time projection when required material is unavailable, context-sensitive Playlist `Clear` behavior (playlist view: modal options; designer view: immediate designer reset), and prep-mode playlist-click start that begins immediately on the selected track without a lead-in cortina and then continues through the remaining playlist sequence. Missing: next-up visualization and multi-client state sync. |
...
| UI-050 | Configuration panel | Yes | Partial | Done: Library/System/Playlist/Display Board tabs, diagnostics paths, waveform test, playback-leveling diagnostics log viewer, data-readiness summary for missing duration/loudness/trim/errors/waveforms, and legacy-section readiness verification button with pass/warn/fail result. Missing: gain target controls and UI visibility toggles. |
