...
| FR-050 | Playback state machine | Yes | Partial | Done: sequential renderer playback with resume state, start/end trims applied, playlist-click prep start behavior, and scheduled cortina/tanda gap handling. Missing: deterministic engine abstraction, multi-client sync, richer scheduling visibility. |
...
| UI-012 | Playlist timeline | Yes | Partial | Done: active tanda expansion, active track highlight, estimated start times, idle auto-centering of the current tanda during playback, fallback to tanda stored duration when per-track analysis duration is missing, auto-fill placeholder tandas (required size/style) with mismatch warnings using 9-minute assumed duration for end-time projection when required material is unavailable, context-sensitive Playlist `Clear` behavior (playlist view: modal options; designer view: immediate designer reset), and prep-mode playlist-click start that begins immediately on the selected track without a lead-in cortina. Missing: next-up visualization and multi-client state sync. |
