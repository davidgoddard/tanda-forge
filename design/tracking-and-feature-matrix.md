...
| FR-001.2 | Background analysis | Yes | Partial | Done: tags, silence offsets, loudness (loudnorm) with tolerant parsing of malformed output, scan reuse guard that forces re-analysis for provisional legacy-import metadata, and analysis-pipeline version invalidation so newer app builds do not silently reuse stale persisted analysis. Missing: batch queue persistence, retry policy, analysis scheduling. |
| FR-001.3 | Progress + resume + deletion handling | Yes | Yes | Done: progress, skip unchanged scan work, missing-file cleanup, persistent user-initiated logical track deletion with optional source-file removal, dependent tanda/playlist invalidation, restart-safe reruns that reuse finished cache work while retrying incomplete duration/loudness/gain outputs, and canonical compressed-companion readiness auditing so setup/verify/playback all evaluate the same eligible-track set and cache-path rule. Persisted in-flight job queues are not required scope. |
| FR-001.4 | Guided setup and recovery | Yes | Yes | Done: one Library-tab action now provides a resumable scan/build/compression recovery flow, destructive legacy migration is exposed as a separate explicit action, and a standalone DB repair utility can strip appended `instrumental` markers from title/artist while clearing singer and forcing instrumental tracks back into a consistent state. |
| FR-001.5 | System backup and restore | Yes | Yes | Done: Library-tab export/import copies the full application data root with manifest validation and destructive-import confirmation. |
...
| FR-011 | Tanda management | Yes | Yes | Done: tanda designer UI, save/delete to DB, search tab, legacy tandas import without full scan (library.dat + cortinas.dat), waveform PNG reuse, metadata overrides, and tandas-only JSON export with portable track references. |
| FR-020 | Playlist structure | Yes | Yes | Done: playlist UI, start/resume/stop, active tanda expansion, live-mode locking, auto-save/restore, cortina slots and playback integration, persisted playlist cortina slot assignments (auto-planned or manual, restored when the same cortina set is still active), in-playlist tanda editor for ad-hoc creation, target-mark replacement and cross-swap with warnings, clear-options modal (clear vs clear+autofill), expected end-time control (overnight-safe), auto-fill cortina pre-assignment for all required cortina rows when cortinas are enabled, and manual playlist save/import in Playlist settings for JSON and grouped M3U/M3U8 interchange. |
| FR-021 | Rule-based playlists | Yes | Partial | Done: sequence/style-map guided auto-fill with fallback ad-hoc tanda synthesis, title de-duplication, grouped M3U/M3U8 import/export for best-effort tanda interchange, and contiguous `group-title` / `EXTGRP` M3U import into tanda snapshots. Missing: user-configurable generation weights/constraints and structured export of rule sets. |
...
| FR-030 | Cortina handling | Yes | Yes | Done: separate cortina roots, set naming incl. Default, playlist set selection, queue shuffle/refill, playback between tandas + start/end, picker modal, persisted playlist slot assignments that are invalidated on cortina-set change while new slots continue to draw from the active set, play-all/stop-now controls with duration feedback, and dedicated cortina playback level control. |
...
| FR-091 | Fuzzy, fault-tolerant search | Yes | Partial | Done: trigram/token fuzzy scoring, token edit-distance bonus, short-token guardrails, whole-query token-coverage scaling for lookup searches, first-class DJ notes scoring with lower-weight album metadata, implicit token parsing (year/tempo/text), quoted-phrase boost, lookup-vs-similarity ranking profiles (numeric intent triggers similarity), proximity scoring for year/tempo with missing-metadata fallback, deterministic component tie-break ranking, configurable min score, numeric query handling, and alias-expanded artist/orchestra matching using canonical+variant metadata. Missing: configurable accent-sensitive toggle. |
...
| UI-012 | Playlist timeline | Yes | Partial | Done: active tanda expansion, active track highlight, estimated start times, idle auto-centering of the current tanda during playback, fallback to tanda stored duration when per-track analysis duration is missing, auto-fill placeholder tandas (required size/style) with mismatch warnings using 9-minute assumed duration for end-time projection when required material is unavailable, context-sensitive Playlist `Clear` behavior (playlist view: modal options; designer view: immediate designer reset), prep-mode playlist-click start that begins immediately on the selected track without a lead-in cortina and then continues through the remaining playlist sequence, guarded Live-mode tanda-start when idle, confirmed Live-mode one-off track playback when idle, resumable live-performance stop points that replay the same cortina on resume, and marked-last countdown stopping with a `tandas before end` field plus localized display-board countdown text. Missing: next-up visualization and multi-client state sync. |
...
| UI-050 | Configuration panel | Yes | Yes | Done: Library/System/Playlist/Display Board tabs, diagnostics paths with ffmpeg/ffprobe source reporting (bundled/custom/PATH), custom FFmpeg tools-folder override, waveform test, playback-leveling diagnostics log viewer, data-readiness summary for missing duration/loudness/errors/waveforms plus informational zero-trim counts and visible compression eligible/ready/missing counts, guided library setup flow with an explicit optional legacy branch that rejoins before analysis, separate final readiness-verification card, guided startup/recovery flow with recommended/manual visual grouping, manual-tool outer container, unified startup-card progress + rough ETA, phase tracker completion state for `Done`, rerun-safe guidance copy, tandas-only export, playlist save/import actions with M3U guidance, system export/import actions, cache-management actions, and track-editor compressed-path lookup that reflects actual current-profile cache readiness instead of only renderer-session state. Gain-target controls and generic UI-visibility toggles are no longer required scope. |
...
- FR-021: rule-based playlists (weights/constraints tuning and structured rule export still pending).
