...
| UI-012 | Playlist timeline | Yes | Partial | Done: active tanda expansion, active track highlight, estimated start times, idle auto-centering of the current tanda during playback, fallback to tanda stored duration when per-track analysis duration is missing, and auto-fill placeholder tandas (required size/style) with mismatch warnings using 9-minute assumed duration for end-time projection when required material is unavailable. Missing: next-up visualization and multi-client state sync. |
