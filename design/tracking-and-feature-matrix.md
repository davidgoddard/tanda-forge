...
| FR-050 | Playback state machine | Yes | Partial | Done: sequential renderer playback with resume state, start/end trims applied, playlist-click start from the selected playlist track in prep/live when idle, natural continuation through the remaining sequence, and scheduled cortina/tanda gap handling. Missing: deterministic engine abstraction, multi-client sync, richer scheduling visibility. |
